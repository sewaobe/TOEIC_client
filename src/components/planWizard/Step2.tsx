...
export const EndDateStep: FC<IEndDateStep> = ({ score }) => {
    ...
    // Cảnh báo nếu vượt quá mức trung bình thời gian học
    const dispatch = useDispatch();
    if (weeklyHours > 16) {
        dispatch(showSnackbar({ message: `Khối lượng học ${weeklyHours} giờ/tuần vượt quá mức tối đa ${maxWeekly} giờ/tuần. Có thể quá tải.`, severity: "warning" }))
    }
    else if (weeklyHours < 3) {
        dispatch(showSnackbar({ message: `Khối lượng học (~${weeklyHours} giờ/tuần) quá ít. Tối thiểu nên từ ${minWeekly.toFixed(2)} giờ/tuần.`, severity: "warning" }))
    }
