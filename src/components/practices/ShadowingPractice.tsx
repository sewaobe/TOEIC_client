    ...
    try {
      ...
      const draw = () => {
        ...
        rings.forEach((r) => {
          ...
          r.alpha -= 0.015
          const g = ctx.createRadialGradient(WIDTH / 2, HEIGHT / 2, 0, WIDTH / 2, HEIGHT / 2, r.radius)
      ...
      setMicError("⚠️ Không thể truy cập micro. Hãy cấp quyền cho trình duyệt.")
      setIsTesting(false)
      onMicReady(false)
    }
  }
    ...
    >
      <Card sx={{ p: 4, borderRadius: 4, textAlign: "center" }}>
        ...
        {!isPlaying ? (
          ...
          <Button variant="outlined" startIcon={<PauseIcon />} onClick={pauseAudio}>
            Tạm dừng
          </Button>
        )}
...
const SpeakingStep = ({
  ...
  const handleToggleRecording = async () => {
    ...
    recorder.ondataavailable = (e) => chunks.push(e.data)
    recorder.onstop = () => setRecordedBlob(new Blob(chunks, { type: "audio/webm" }))
    recorder.start()
    setIsRecording(true)
      ...
      <Card sx={{ p: 4, textAlign: "center", borderRadius: 4 }}>
        ...
        {!hasRecorded && (
          <Typography mt={2} color="text.secondary">
            {isRecording ? "Đang thu âm... Ấn dừng khi bạn nói xong 🎙️" : "Ấn mic để bắt đầu nói!"}
          </Typography>
        )}
...
const FeedbackStep = ({ feedback, onNext }: { feedback: any; onNext: () => void }) => {
  ...
  const fmt = (v?: number) => (v !== undefined ? `${Math.round(v * 100)}%` : "—");
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      >
        ...
        >
          {[
            { label: "Similarity", value: fmt(similarity_score), color: "#2563eb" },
            { label: "Accuracy", value: fmt(accuracy_score), color: "#9333ea" },
            { label: "Fluency", value: fmt(fluency_score), color: "#10b981" },
            { label: "Intonation", value: fmt(intonation_score), color: "#f59e0b" },
          ].map((item) => (
            ...
            {sentence_alignment.map((s: any, i: number) => (
              <Box key={i} sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ color: "#1e3a8a" }}>
                  <b>Native:</b>{" "}
                  {s.native_sentence.split(" ").map((word: string, j: number) => {
                    const diff = s.word_diffs?.find((d: any) => d.word === word);
                    let color = "inherit";
                    if (diff?.status === "missing") color = "#dc2626";
                    else if (diff?.status === "mispronounced") color = "#f59e0b";
                    else if (diff?.status === "extra") color = "#9333ea";
                    return (
                      <span key={j} style={{ color, fontWeight: diff ? 600 : 400 }}>
                        {word}{" "}
                      </span>
                    );
                  })}
        ...
        {pronunciation_feedback && (
          ...
          >
            ...
            {pronunciation_feedback.mispronounced?.length > 0 && (
              <Typography>
                ❌ <b>Từ phát âm sai:</b> {pronunciation_feedback.mispronounced.join(", ")}
              </Typography>
            )}

            {pronunciation_feedback.missing_words?.length > 0 && (
              <Typography>
                ⚠️ <b>Từ bị bỏ sót:</b> {pronunciation_feedback.missing_words.join(", ")}
              </Typography>
            )}

            {pronunciation_feedback.extra_words?.length > 0 && (
              <Typography>
                💬 <b>Từ nói thừa:</b> {pronunciation_feedback.extra_words.join(", ")}
              </Typography>
            )}
          </Box>
        )}

        {/* === 3️⃣ Tóm tắt lỗi nổi bật === */}
...
export default function ShadowingContent({ lesson }: ShadowingContentProps) {
  ...
  const handleAnalyze = async (blob: Blob) => {
    ...
    try {
      ...
      const currentSeg = segments[currentIndex]; // 👈 lấy timing hiện tại

      const result = await geminiService.analyzeShadowing(
        userUrl,
        lesson.audio_url!,        // vẫn gửi URL gốc
        {
          level: lesson.level,
          segmentIndex: currentIndex,
          nativeStart: currentSeg.start,  // 👈 gửi thêm
          nativeEnd: currentSeg.end,      // 👈 gửi thêm
          shadowing: {
            _id: lesson._id,
            title: lesson.title,
            level: lesson.level,
  ...
  const handleSubmitFast = (blob: Blob) => {
    // 1️⃣ Upload để lưu cloud, không cần chấm
    uploadAuto(blob).then((url) => {
      console.log("Saved user audio:", url)
      handleNextSegment() // chuyển câu kế tiếp
    })
  }
