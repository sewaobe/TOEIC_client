...
import { getDurationMinutes, getQuestionSet, grade } from '../constants/questionBank';
...
export const useLessonViewModel = (dayId: string, week: string) => {
    ...
    const [currentLesson, setCurrentLesson] = useLocalStorage<LessonItem | null>('current_lesson', null);
    ...
    const startLesson = () => setActivityStatus('studying');
    const finishLesson = () => {
    ...
    // Effect: Lấy dữ liệu riêng cho từng bài học (ví dụ: flashcard)
    React.useEffect(() => {
        ...
        const fetchLessonSpecificData = async () => {
            if (currentLesson.type === 'flash_card') {
                try {
                    const { topic_id, dataVocabularies } = await flashCardService.getFlashcardById(currentLesson.id);
    ...
    // =================================================================
    const selectLesson = React.useCallback(async (targetLesson: LessonItem) => {
        if (targetLesson.status === 'lock' || targetLesson.id === currentLesson?.id) return;

        setIsReviewMode(false);
        setCurrentLesson(targetLesson);
    }, [currentLesson?.id, setCurrentLesson]);

    const completeAndGoToNext = React.useCallback((score?: number) => {
        if (!currentLesson) return;
        ...
        const newList: LessonItem[] = lessons.map((lesson, index) => {
            ...
            if (index === currentIndex + 1 && lesson.status === 'lock') return { ...lesson, status: 'in_progress' };
            return lesson;
        });
        setLessons(newList);
