        ...
        await Promise.all(
          answers.map(async (ans) => {
            try {
              ...
              console.error("Failed to fetch group for question", ans.question_id, err);
            }
          })
        );

        const arr: GroupWithAnswers[] = Object.values(fetched)
          .sort((a, b) => {
        ...
        <Box display="flex" gap={2} p={2}>
          ...
          <Box ref={containerRef} sx={{ flex: 1, maxWidth: 960, mx: 'auto' }}>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
              <Stack direction="row" spacing={1} alignItems="center">
                <IconButton size="small" onClick={() => navigate(location.pathname.replace(/\/answers$/, ""))}>
                  <ArrowBackIcon />
                </IconButton>
                <Typography variant="h6">Đáp án chi tiết{testTitle ? `: ${testTitle}` : ""}</Typography>
              </Stack>
            </Stack>
            ...
            {!loading && groups.length > 0 && (
              <Box className="space-y-8">
                {groups.filter((g) => (activePart ? g.part === activePart : true)).map((g, gi, arr) => (
                  <Box key={g._id}>
                    {/* Part anchor */}
                    ...
                    <Box className="bg-white rounded-xl shadow" sx={{ p: 2, maxWidth: 900, mx: 'auto' }}>
                        ...
                        <Box sx={{ mb: 2 }}>
                          {g.imagesUrl.map((url, i) => (
                            <img
                              ...
                              style={{ maxHeight: 380, objectFit: 'contain', border: '1px solid #eee' }}
                            />
                          ))}
                        </Box>
                      )}
                      ...
                      {(g.transcriptEnglish || g.transcriptTranslation) && (
                        <TranscriptBox english={g.transcriptEnglish} translation={g.transcriptTranslation} />)
                      }
                      ...
                      {g.questions
                        ...
                        .map((q) => (
                          <QuestionBlock key={q._id} q={q} ans={answerMap.get(q._id)!} />
                        ))}
                    </Box>
                  </Box>
                ))}
          ...
          <Box sx={{ width: 220, position: "sticky", top: 12, alignSelf: "flex-start" }}>
            <Box className="bg-white rounded-xl shadow" sx={{ p: 2 }}>
              <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1 }}>Điều hướng</Typography>
              {parts.map((p) => {
                const qs = answers
                  .filter((a) => {
                ...
                return (
                  <Box key={p} sx={{ mb: 1.5 }}>
                    <Typography variant="caption" sx={{ fontWeight: 700 }}>Part {p}</Typography>
                    <Box sx={{ display: "flex", gap: 0.5, flexWrap: "wrap", mt: 0.5 }}>
                      {qs.map((a) => {
                        const status = a.selectedOption === ""
                          ? "skipped"
                          : a.isCorrect
                            ? "correct"
                            : "wrong";
                        let sxStyle: any = { minWidth: 36, p: 0, fontSize: 12, borderColor: "#D6D6D6", color: "#555" };
                        if (status === "correct") sxStyle = { ...sxStyle, bgcolor: "#D1FADF", color: "#065F46", border: "none" };
                        else if (status === "wrong") sxStyle = { ...sxStyle, bgcolor: "#FEE2E2", color: "#B91C1C", border: "none" };
                        else if (status === "skipped") sxStyle = { ...sxStyle, bgcolor: "#FEF3C7", color: "#92400E", border: "none" };
                        return (
...
function QuestionBlock({ q, ans }: { q: ExamQuestion; ans: RawAnswer }) {
  ...
  const status = ans.selectedOption === "" ? "skipped" : ans.isCorrect ? "correct" : "wrong";
  ...
  const chipByStatus: any = {
    correct: { label: 'Đúng', color: 'success' as const, variant: 'filled' as const },
    wrong: { label: 'Sai', color: 'error' as const, variant: 'filled' as const },
    skipped: { label: 'Bỏ qua', color: 'warning' as const, variant: 'filled' as const },
  };
  return (
    <Box
    ...
    >
        ...
        return (
          <Box key={key} sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.5 }}>
            <Typography
              sx={{
                fontSize: 15,
                color: isCorrect ? "success.main" : isSelected && !isCorrect ? "error.main" : "text.primary",
      ...
      {q.explanation && (
        <Box sx={{ mt: 0.5 }}>
          <Button endIcon={openExplain ? <ExpandLessIcon /> : <ExpandMoreIcon />} size="small" onClick={() => setOpenExplain((v) => !v)}
            sx={{ color: 'text.primary', fontWeight: 600 }}>
            Giải thích chi tiết đáp án
          </Button>
          <Collapse in={openExplain}>
            <Typography sx={{ whiteSpace: "pre-line", mt: 0.5, bgcolor: '#F9FAFB', p: 1.5, borderRadius: 1 }}>{q.explanation}</Typography>
          </Collapse>
        </Box>
      )}
    </Box>
...
function TranscriptBox({ english, translation }: { english?: string; translation?: string }) {
  const [open, setOpen] = useState(false);
  if (!english && !translation) return null;
  return (
    <Box sx={{ mt: 1 }}>
      <Button size="small" endIcon={open ? <ExpandLessIcon /> : <ExpandMoreIcon />} onClick={() => setOpen((v) => !v)}
        sx={{ fontWeight: 600 }}>
        Hiện Transcript
      </Button>
      <Collapse in={open}>
        {english && (
          <Typography sx={{ whiteSpace: "pre-line", mt: 0.5, bgcolor: '#E5F4FF', p: 1.25, borderRadius: 1 }}>{english}</Typography>
        )}
        {translation && (
          <Typography sx={{ whiteSpace: "pre-line", color: "text.secondary", mt: 0.75, bgcolor: '#F3F4F6', p: 1.25, borderRadius: 1 }}>{translation}</Typography>
        )}
      </Collapse>
    </Box>
  );
