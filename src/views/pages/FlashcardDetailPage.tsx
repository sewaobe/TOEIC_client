...
import CreateFlashcardItemModal, {
  FlashcardItem,
} from "../../components/modals/CreateFlashcardItemModal";
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          ...
          <IconButton onClick={() => f._id && handleDelete(f._id)} size="small">
...
const FlashcardDetail: React.FC = () => {
  ...
  // Fetch topic info từ API khi localStorage trống (ví dụ: link từ chatbot)
  useEffect(() => {
    const fetchTopicInfo = async () => {
      if (!flashcardInfo && topicId) {
        try {
          const info = await topicService.getTopicInfo(topicId);
          setFlashcardInfo(info);
          localStorage.setItem("flashcardInfo", JSON.stringify(info));
        } catch (error) {
          console.error("Không thể lấy thông tin topic:", error);
        }
      }
    };
    fetchTopicInfo();
  }, [topicId, flashcardInfo]);
  ...
  const handleSaveItem = async (data: FlashcardItem) => {
    const finalImageUrl = await uploadToCloudinaryFromString(
      data.image || null
    );
    if (!data.word.trim()) {
    ...
    const fetchSession = async () => {
      if (topicId) {
        ...
        const currentSession = sessions.items.find(
          (session: any) => session.topic._id === topicId
        );
        if (currentSession) {
      ...
      >
        ...
        <Paper sx={{ p: 2, bgcolor: "#DFFBF0", mb: 2 }}>
          <Typography variant="body2">
            Chú ý: Audio mặc định là tiếng Anh-Anh và Anh-Mỹ. Các ngôn ngữ khác
        ...
        <Button
          ...
          onClick={async () => {
            if (flashcards.length === 0) {
              toast.error(
                "Danh sách từ trống. Vui lòng thêm từ để bắt đầu luyện tập."
              );
              return;
            }
            navigate("practice");
            ...
            try {
              if (topicId) {
                const res = await toast.promise(
                  topicService.updateTopicVocabulary(topicId, data),
                  {
