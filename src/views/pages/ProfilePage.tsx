  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const user = await userService.getUserByUsername("thanha17"); // hoặc theo token
            ...
            <motion.div variants={itemVariants} className="md:col-span-2">
              <CourseHistory
                courses={userData.progress.map((p: any) => ({
                  id: p._id,
                  name: p.learningPaths_id.title,
                  completion: Math.round(
                    (p.completedLessons_id.length /
                      p.learningPaths_id.lessons.length) *
                      100
                  ),
                }))}
              />
