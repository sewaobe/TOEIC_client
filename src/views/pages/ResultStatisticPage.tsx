...
import TestHistoryTable, {
  TestHistory,
} from "../../components/result-statictis/TestHistoryTable";
...
export default function ResultStatisticPage() {
  ...
  const [scoreTrend, setScoreTrend] = useState<
    { month: string; listening: number; reading: number }[]
  >([]);
  ...
  const fetchDataTestHistory = async () => {
    setIsLoading(true);
    try {
      const res = await progressService.getUserTestStatistics(
        filterYear,
        filterMonth
      );
      if (!res?.data) return;
      ...
      const converted: TestHistory[] = res.data.map(
  ...
  const fetchPartAccuracy = async () => {
    setIsLoading(true);
    try {
      const res = await progressService.getPartAccuracyStats(
        filterYear,
        filterMonth
      );
      setPartAccuracy(res);
  ...
  const getStatsCards = () => {
    ...
    if (filterMonth !== "all") {
      filteredData = overview.filter(
        (o) => o.month === filterMonth && o.year === filterYear
      );
    } else {
    ...
    const avgListening =
      filteredData.length > 0
        ? Math.round(
            filteredData.reduce((sum, o) => sum + o.avgListening, 0) /
              filteredData.length
          )
        : 0;
    ...
    const avgReading =
      filteredData.length > 0
        ? Math.round(
            filteredData.reduce((sum, o) => sum + o.avgReading, 0) /
              filteredData.length
          )
        : 0;
    ...
    const getPrevPeriod = () => {
      if (filterMonth === "all") {
        ...
        return overview.filter(
          (o) => o.month === prevMonth && o.year === prevYear
        );
      }
    };

    const prevData = getPrevPeriod();
    const prevAvgListening =
      prevData.length > 0
        ? Math.round(
            prevData.reduce((sum, o) => sum + o.avgListening, 0) /
              prevData.length
          )
        : avgListening;

    const prevAvgReading =
      prevData.length > 0
        ? Math.round(
            prevData.reduce((sum, o) => sum + o.avgReading, 0) / prevData.length
          )
        : avgReading;
    ...
    return [
      {
        ...
        trendValue:
          listeningDiff >= 0 ? `+${listeningDiff}` : `${listeningDiff}`,
      },
      {
        label: "Reading",
          ...
          {isLoading ? (
            ...
            <Card sx={{ borderRadius: 3 }}>
              <CardHeader
                ...
                subheader={`Theo dõi tiến độ ${
                  filterMonth === "all" ? "cả năm" : `tháng ${filterMonth}`
                } `}
              />
              <CardContent>
                <PracticeSkillPanel
